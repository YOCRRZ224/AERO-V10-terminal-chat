...
def handle_chat(data):
    ...
    # 🎵 Music Commands (handled before broadcasting)
    if text.startswith('/') or text.startswith('!'):
        ...
        if cmd in ('/play', '!play') and arg:
            ...
            if track:
                ...
                if not NOW_PLAYING:
                    _start_next_track()
                socketio.emit('queue_update', MUSIC_QUEUE, to='global')
                socketio.emit('now_playing', NOW_PLAYING, to='global')
            return  # prevent command from showing as chat
        ...
        # --- /skip or !skip ---
            ...
            if MUSIC_QUEUE:
                ...
                socketio.emit('system', {
                    'msg': f'{session.get("display", user)} skipped the track',
                    'time': datetime.now().strftime('%H:%M')
        }, to='global')
            return
